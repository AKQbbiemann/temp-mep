...
<template>
  <div class="sub-container-gray flex flex-col">
    <div class="flex justify-between">
      <LoadProfileSearch
        @newCompetence="attachCompetence($event, props.item.id)"
      />
      <div class="w-[70px] flex justify-between align-middle">
        <UIcon
          ...
          dynamic
          @click="emit('editLoadProfile', props.item.id)"
        />
        ...
        <UIcon
          ...
          dynamic
          @click="emit('deleteLoadProfile', props.item.id)"
        />
      </div>
    </div>
    <div class="self-center flex justify-center w-full">
      <UCarousel
        ...
        :items="[
          ...
          {
            fullTimeEmployees: {
              fte: props.item.full_time_employees,
              profileId: props.item.id,
              employeeChanges: props.item.employee_changes,
            },
          },
        ]"
        :ui="{
        ...
        <template v-if="Object.keys(item)[0] === 'loadProfile'">
          ...
          >
            <load-profile-competence
              @newCompetence="attachCompetence($event, item['loadProfile'].id)"
            >
              <template #competence>
                <template
            ...
            <span class="text-sm pt-4">
              {{
                $t("FTE_FOR_TODAY") + ": " + getTodaysFTE(item["loadProfile"])
              }}
            </span>
          </div>
        </template>
        <div v-else class="border-t-2 mt-8 w-full">
          <div class="flex flex-col justify-start align-middle pt-10 w-full">
            <UButton
              type="button"
              color="akq-green"
              ...
              @click="
                emit('addLoadProfileFTE', item['fullTimeEmployees'].profileId)
              "
            >
